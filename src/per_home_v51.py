...
from .feature_pruning import prune_constant_features
from .model_zoo_v51 import get_regularized_model_candidates
...
logger = get_logger("per_home_v51")
...
def train_per_home_v51(features_df, output_dir, random_state=42, min_class_samples=30):
    logger.info("Training per-home V5.1 models")

    final_rows = []

    for dataset_name, ds in features_df.groupby("dataset"):
        logger.info(f"[{dataset_name}] preparing V5.1 training")
        ...
        ds, kept_cols, removed_cols = prune_constant_features(ds)
        ...
        candidates = get_regularized_model_candidates(random_state=random_state)

        best_model_name = None
        best_model = None
        best_val_macro = -1
        best_val_metrics = None
        best_uses_encoded = False

        for model_name, model in candidates.items():
            logger.info(f"[{dataset_name}] training candidate={model_name}")

            uses_encoded = model_name in {"xgboost_reg", "lightgbm_reg", "catboost_reg"}
            ...
            if uses_encoded:
                ...
                fit_model(model_name, model, X_train, y_train, sample_weight=None)
            ...
            if val_metrics["macro_f1"] > best_val_macro:
                ...
                best_uses_encoded = uses_encoded

        logger.info(f"[{dataset_name}] best model={best_model_name} with val_macro_f1={best_val_macro:.4f}")

        # calibration
        if hasattr(best_model, "predict_proba"):
            if best_uses_encoded:
                calibrated = calibrate_model(best_model, X_val, y_val_enc, method="sigmoid")
            else:
                calibrated = calibrate_model(best_model, X_val, y_val, method="sigmoid")
        else:
        ...
        if best_uses_encoded:
            ...
            test_pred_enc = proba.argmax(axis=1)
            ...
            if hasattr(calibrated, "predict_proba"):
                proba = calibrated.predict_proba(X_test)
                class_to_idx = {c: i for i, c in enumerate(calibrated.classes_)}
        ...
        joblib.dump(calibrated, output_dir / f"v51_best_model_{dataset_name}.pkl")
        joblib.dump(le, output_dir / f"v51_label_encoder_{dataset_name}.pkl")
        joblib.dump(kept_cols, output_dir / f"v51_kept_features_{dataset_name}.pkl")

        importance_path = output_dir / f"v51_feature_importance_{dataset_name}.csv"
        save_feature_importance(best_model, X_train.columns.tolist(), importance_path, top_n=60)

        report_path = output_dir / f"v51_report_{dataset_name}.txt"
        ...
        final_rows.append({
            ...
            "num_features_kept": len(kept_cols),
            "num_features_removed": len(removed_cols),
        })
    ...
    if not summary.empty:
        summary.to_csv(output_dir / "v51_summary.csv", index=False)
        logger.info(f"Saved V5.1 summary to {output_dir / 'v51_summary.csv'}")
    return summary
